...
function theme_enqueue_scripts() {
     ...
     */
    wp_enqueue_script( 'frontendjs', get_template_directory_uri() . '/custom.js', array('jquery'), null, true );
    wp_localize_script( 'frontendjs', 'demojs',
        array( 
...
add_action( 'init', 'products_color_custom_taxonomy', 0 );
 
//create a custom taxonomy name it "type" for your posts
function products_color_custom_taxonomy() {
...
add_action( 'init', 'products_size_custom_taxonomy', 0 );

function products_size_custom_taxonomy() {
  ...
  register_taxonomy('sizes',array('products'), array(
    'hierarchical' => true,
    'labels' => $size,
    'show_ui' => true,
    'show_admin_column' => true,
...
if ( ! function_exists( 'hongo_child_ajax_product' ) ) {
    function hongo_child_ajax_product( $args = array() ) {
        if( ! empty( $_POST['parentID'] && $_POST['taxonomyName'] ) ) {
            $result = get_products_using_ajax( $_POST['parentID'],  $_POST['taxonomyName'] );
            if( empty( $result ) ) {
                $result = '<span>' . esc_html__( 'No data found!!', 'demo-interview' ) . '</span>';
            }
            echo $result;
        }
        die();
...
add_action( 'wp_ajax_get_ajax_products', 'hongo_child_ajax_product' );
add_action( 'wp_ajax_nopriv_get_ajax_products', 'hongo_child_ajax_product' );

if ( ! function_exists( 'get_products_using_ajax' ) ) {
    function get_products_using_ajax( $parentID, $taxonomyName ) {
    	$output = '';
		...
		if( ! empty( $products_array ) && ! is_wp_error( $products_array ) ) {
	        foreach ($products_array as $product ) {
				$output .= '<div class="product-list">';
				$output .= '<div>'.get_the_post_thumbnail($product->ID).'</div>';
				$output .= '<div><a href="'.$product->guid.'">'.$product->post_title.'</a></div>';
				$output .= '<div>'.$product->post_excerpt.'</div>';
				$output .= '</div>';
			}
		}
		return $output;
